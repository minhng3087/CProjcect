...
const char* soundex(const char *s)
{
	...
	prev = code[(int)out[0]];//trong so cua chu cai dau tien
	for (i = 1; *s && i < 4; s++) {
		if ((c = code[(int) * s]) == prev) continue;//neu nhu trong so cua chu tiep theo bang prev thi bo qua

		if (c == -1) prev = 0;	/* vowel as separator */ //gan chu cai dau tien = space
		else if (c > 0) {
			out[i++] = c + '0';
			prev = c;
		}
...
int main(){
	...
	soundexTree = btcrt("../data/soundex.dat",0,1);
